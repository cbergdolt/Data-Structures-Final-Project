/* musicMatch.cpp
 * Main function
 * Uses data files generated by python script
 * Includes match.h to process the info
*/
...
void analyze_artists(User user1, User user2, char * neighborQuantity) {
    ...
    // Set up data structures for djiktra's algorithm to find distance 
    //  between two artists and intermediate artists
        ...
        if (! marked.count(curr.name)) {
            ...
            for (it = neighbors.begin(); it != neighbors.end(); it++) { 
                ...
                if (end == *it) {
                    ...
                    break;  // Only get the first match (even if another match on the same level)
                }
